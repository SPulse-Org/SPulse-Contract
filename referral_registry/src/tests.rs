...
fn test_credit_with_referrer() {
    ...
    // Referrer got 3 leaderboard bonus points (referrer registered with no
    // referrer of their own, so they get no welcome bonus — issue #74 sybil
    // guard). Points are queued and applied by claim_pending_rewards.
    let lb_client = leaderboard::LeaderboardContractClient::new(&t.env, &t.leaderboard_id);
...
fn test_referrer_bonus_points_accumulate() {
    ...
    // 3 credits → 3 × 3 = 9 bonus pts for referrer. The referrer registered
    // without a referrer of their own, so they get no welcome bonus (issue
    // #74 sybil guard) — only the per-credit referral points.
    t.client.credit(&t.market, &user, &5_000_000_i128);
    t.client.credit(&t.market, &user, &5_000_000_i128);
    t.client.credit(&t.market, &user, &5_000_000_i128);

    // Points are queued and applied by claim_pending_rewards.
    let lb_client = leaderboard::LeaderboardContractClient::new(&t.env, &t.leaderboard_id);
    assert_eq!(lb_client.get_points(&referrer), 0); // queued, not yet applied
    lb_client.claim_pending_rewards(&referrer);
...
fn test_registry_pause_blocks_registration_and_credit_then_resume() {
    ...
    assert!(!t.client.paused());
    t.client
        .register_referral(&user, &String::from_str(&t.env, "User"), &no_ref);
    assert!(t.client.is_registered(&user));
}
...
fn setup_with_incompatible_leaderboard() -> TestSetup {
    ...
    // The registry mints the 1 PULSE welcome bonus directly, so it must be an
    // authorized minter even when the leaderboard is incompatible.
    pulse_token::PULSETokenContractClient::new(&env, &token_id).set_minter(&referral_id);
...
fn test_credit_survives_incompatible_leaderboard() {
    ...
    let user = Address::generate(&t.env);
    let referrer = Address::generate(&t.env);
    // Issue #99: credit() refuses to pay an unregistered referrer, so the
    // referrer must register first. register_referral only *queues* the bonus
    // via try_invoke_contract (and the PULSE mint is independent of the
    // leaderboard), so it succeeds even against an incompatible leaderboard.
    t.client
        .register_referral(&referrer, &String::from_str(&t.env, "RefKing"), &None);
    t.client.register_referral(
        &user,
...
fn test_migrate_user_writes_profile_removes_legacy() {
    ...
    assert_eq!(
        t.client.get_referrer(&legacy_user),
        Some(legacy_ref.clone())
    );
