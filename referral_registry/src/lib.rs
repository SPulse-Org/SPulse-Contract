...
const UPGRADE_DELAY_SECS: u64 = 86_400; // 24h timelock
...
impl ReferralRegistryContract {
    ...
    pub fn propose_upgrade(
        ...
        if env.storage().instance().has(&DataKey::PendingUpgrade) {
            return Err(ReferralError::UpgradeExists);
        }
    ...
    pub fn approve_upgrade(env: Env, caller: Address) -> Result<u32, ReferralError> {
        caller.require_auth();
        Self::require_governor(&env, &caller)?;
        let mut pending: PendingUpgradeChange = env
            ...
            .ok_or(ReferralError::NoUpgrade)?;
        if Self::approver_index(&pending.approvers, &caller).is_some() {
            return Err(ReferralError::AlreadyApproved);
        }
    ...
    pub fn execute_upgrade(env: Env, caller: Address) -> Result<(), ReferralError> {
        caller.require_auth();
        Self::require_governor(&env, &caller)?;
        ...
        {
            return Err(ReferralError::UpgradeTooSoon);
        }
        ...
        if pending.approvers.len() < threshold {
            return Err(ReferralError::InsufficientApprovals);
        }
    ...
    pub fn cancel_upgrade(env: Env, caller: Address) -> Result<(), ReferralError> {
        caller.require_auth();
        Self::require_governor(&env, &caller)?;
        if !env.storage().instance().has(&DataKey::PendingUpgrade) {
            return Err(ReferralError::NoUpgrade);
        }
        env.storage().instance().remove(&DataKey::PendingUpgrade);
        env.events().publish(
    ...
    pub fn add_governor(env: Env, admin: Address, governor: Address) -> Result<(), ReferralError> {
        ...
        if count >= MAX_GOVERNORS {
            return Err(ReferralError::InvalidThreshold);
        }
    ...
    pub fn remove_governor(env: Env, admin: Address, governor: Address) -> Result<(), ReferralError> {
        Self::require_admin(&env, &admin)?;
        admin.require_auth();
        let count: u32 = env
            .storage()
        ...
        if !env.storage().persistent().get(&key).unwrap_or(false) {
            return Err(ReferralError::NotGovernor);
        }
    ...
    fn require_governor(env: &Env, caller: &Address) -> Result<(), ReferralError> {
        if env
            .storage()
            .persistent()
