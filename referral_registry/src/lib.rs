...
impl ReferralRegistryContract {
    ...
    pub fn set_xlm_sac(env: Env, admin: Address, xlm_sac: Address) -> Result<(), ReferralError> {
        ...
        env.storage()
            ...
            .set(&DataKey::XlmSacContract, &xlm_sac);
        env.events()
            .publish((Symbol::new(&env, "xlm_sac_set"), admin), xlm_sac);
        Ok(())
    }
    ...
    pub fn register_referral(
        ...
        if let Some(ref ref_addr) = referrer {
            ...
            // Issue #74 (sybil guard): the welcome bonus (5 pts + 1 PULSE) is
            // only awarded when the registrant actually brings a referrer —
            // otherwise self-registration farms would mint unboundedly. The
            // points are queued on the leaderboard (applied via
            // claim_pending_rewards in a separate tx) so registration stays
            // cheap; the 1 PULSE is minted directly here because the registry
            // is an authorized minter for this wiring. Both are optional side
            // effects: registration remains successful if the leaderboard is
            // paused, unavailable, or exceeds the remaining invocation budget.
        ...
        env.events()
            .publish((Symbol::new(&env, "referral_registered"), user), referrer);
        Ok(())
    }
    ...
    pub fn credit(
        ...
        match referrer {
            Some(ref_addr) => {
                ...
                if !Self::is_registered(env.clone(), ref_addr.clone()) {
                    // Issue #76: hold the fee as admin-withdrawable surplus
                    // instead of round-tripping it back to the caller — the
                    // market never escrowed it, so a refund would look like a
                    // payment and be exploitable.
                    if referral_fee > 0 {
            ...
            None => {
                ...
                env.events()
                    .publish((Symbol::new(&env, "referral_missed"), user), referral_fee);
                Ok(false)
            }
        }
    }
    ...
    /// Issue #76/#77: admin withdraws fees that could not be paid out (no
    /// referrer, or referrer unregistered / at cap). Surplus is a storage
    /// ledger, not an escrowed balance, so the admin must send XLM to this
    /// contract before withdrawing.
    pub fn withdraw_surplus_fees(
        ...
        env.storage()
            .persistent()
            .set(&DataKey::SurplusFees, &0_i128);
        Ok(amount)
    }
    ...
    pub fn migrate_user(env: Env, admin: Address, user: Address) -> Result<(), ReferralError> {
        ...
        env.storage()
            .persistent()
            .remove(&DataKey::Registered(user.clone()));
        env.storage()
            .persistent()
            .remove(&DataKey::DisplayName(user.clone()));
