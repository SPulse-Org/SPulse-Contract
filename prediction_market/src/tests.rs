...
fn test_execute_set_config_without_pending_rejected() {
    let t = setup();
    // No pending config - should fail immediately.
    t.client.execute_set_config(&t.admin);
}
...
fn test_cancel_preserves_unrelated_market_fees() {
    ...
    t.client.place_bet(&user_a, &id_a, &true, &100_0000000_i128);
    t.client.place_bet(&user_b, &id_b, &true, &50_0000000_i128);
    assert_eq!(t.client.get_accumulated_fees(), 3_0000000);
...
// ── #1: referrer-backed cancellation is solvent ──────────────────────────
#[test]
...
// `credit` (which the market invokes mid-`place_bet`, while its reentrancy
// lock is held). The re-entrant bet MUST be rejected.
...
impl EvilReferral {
    ...
    pub fn credit(env: Env, market: Address, user: Address, amount: i128) -> bool {
        ...
        assert!(
            result.is_err(),
            "re-entrant place_bet must be rejected"
        );
